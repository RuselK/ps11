...
export default function Contact() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (response.ok) {
        setSubmitMessage("Ваше сообщение успешно отправлено. Мы свяжемся с вами в ближайшее время.")
        reset()
      } else {
  ...
  return (
    <section id="contact" className="py-20 bg-white">
      <div className="container mx-auto px-6 xl:px-48">
        <h2 className="text-4xl font-bold text-center text-primary mb-12">Свяжитесь с нами</h2>
        <div className="max-w-lg mx-auto">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex items-center">
              <input
                type="checkbox"
                id="consent"
                {...register("consent", { required: "Вы должны согласиться с политикой конфиденциальности" })}
                className="mr-2"
              />
            ...
            {errors.consent && <p className="text-red-500">{errors.consent.message}</p>}
            <button
              type="submit"
          ...
          {submitMessage && <div className="mt-4 p-4 bg-green-100 text-green-700 rounded-lg">{submitMessage}</div>}
        </div>
      </div>
    </section>
  )
